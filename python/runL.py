...
def main() -> None:
    ...
    nz = 100
    zs = np.array([0.0, 10.0, *list(rng.random(nz - 2) * 10.0)], dtype=float)
    zs = np.sort(zs.reshape(-1))

    SFT = 1.1

    def gfn(t: np.ndarray, s: np.ndarray) -> np.ndarray:
        # MATLAB: exp(-t*(s'+SFT)).*(s'+SFT)
        sp = s[None, :] + SFT
        return np.exp(-t[:, None] * sp) * sp

    ng = 32
    gs = chebyshev_nodes(ng)
    T = normalize_columns(gfn(zs, gs))
    tol = np.linalg.norm(T, ord="fro") * 1e-8
    ...
    for it in (1, 2):
        if it == 1:
            ...
            xs = np.array([-0.9, 0.0, 0.25, 0.9], dtype=float)
        ws = np.ones_like(xs)
        nx = xs.size

        for is_idx, STD in enumerate([1e-5, 1e-6, 1e-7], start=1):
            us = gfn(zs, xs) @ ws
            ...
            plt.figure()
            plt.stem(xs + SFT, ws, linefmt="b-", markerfmt="bo", basefmt=" ")
            plt.stem(xa + SFT, wa, linefmt="g-", markerfmt="gs", basefmt=" ")
            plt.stem(xb + SFT, wb, linefmt="r-", markerfmt="r^", basefmt=" ")
            plt.title(f"runL it={it} STD={STD:g} relerr={relerr:g}")
            plt.gca().tick_params(labelsize=FS)

            save_eps(f"exL_{it}{is_idx}")
            plt.close()
